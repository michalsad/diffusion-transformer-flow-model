...
class Head(nn.Module):
    ...
    def forward(self, x):
        ...
        k = self.key(x) # (b, dim, head_size)
        q = self.query(x)   # (b, dim, head_size)
        v = self.value(x)   # (b, dim, head_size)

        # Compute attention scores ("affinities")
        w = q @ k.transpose(-2, -1) # (b, dim, head_size) @ (b, head_size, dim) --> (b, dim, dim)
        w = w * self.scale  # Scale weights to variance 1 so their distribution after softmax is not peaked at very few values
        w = F.softmax(w, dim=-1)    # Normalize rows to sum to one

        # Weighted aggregation of the values
        out = w @ v   # (b, dim, dim) @ (b, dim, head_size) --> (b, dim, head_size)
